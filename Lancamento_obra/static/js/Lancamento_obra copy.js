...
function deleteData(id) {
    apiRequest.delete("deletar", objLoaded.name, id, function () {
...
function changeAtividade() {
    ...
    const campos1 = ["horaini3", "horafim3", "diaseguinte", "atividade_id"];
...
submit = {
    post: function (type) {
        apiRequest.post(type, objLoaded.name, getData(), function () {
            $("#table").bootstrapTable("refresh");
        });
    },
    update: function (type) {
        apiRequest.update(type, objLoaded.name, getData(), function () {
            $("#table").bootstrapTable("refresh");
        });
    },
};
...
async function loadSelect(select, filter) {
    const data = await apiRequest.get("select/" + select, filter);
